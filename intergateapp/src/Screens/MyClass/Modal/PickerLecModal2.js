...
export default class PickerLecModal2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  UNSAFE_componentWillMount() {
    // const items = this.state.ownLecItems;
            ...
            <View style={styles.modalWrapper}>
                <View style={styles.modalContent}>
                    <ScrollView
                        ...
                        {
                            this.state.ownLecItems.map((item, index) => {
                                return (
                                    <View style={styles.modalItem} key={index}>
                                        <TouchableOpacity
                                            style={styles.modalItemWrapper}
                                            onPress={() =>
                                                this.props.screenState.setDataLecListModel(TARGET.FROM_LEC, item.memberClassIdx)
                                                // this.props.screenState.setDataLecListModel(TARGET.FROM_LEC, index)
                                            }
                                            >
                                            ...
                                            <View style={styles.modalItemIconSelectedWrapperCenter}>
                                                <CustomTextR
                                                    style={
                                                        // this.props.screenState.selectedLecItemMemberClassIdx === item.memberClassIdx
                                                        // this.props.screenState.selectedLecItemIndex === index
                                                        parseInt(this.props.screenState.selectedMemberClassIdx) === parseInt(item.memberClassIdx)
                    ...
                    <View style={styles.cancelButton}>
                        <TouchableOpacity
                            ...
                            onPress={() => this.props.screenState.toggleModal()}
                            // onPress={() => this.props.screenState.toggleLecListModal(TARGET.FROM_LEC)}
                            >
