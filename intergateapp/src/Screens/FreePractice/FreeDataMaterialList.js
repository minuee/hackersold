...
export default class FreeDataMaterialList extends Component {
    constructor(props) {
        super(props);
        this.state={
            currentPage: 1,
    ...
    getMaterialList = async() => {
        ...
        const url = DEFAULT_CONSTANTS.apiAdminDomain
            + '/v1/app/freeStudy/studyFile/' + JSON.parse(this.state.myInterestCode).code
            + '?page=' + this.state.currentPage + '&paginate=' + paginate
            + (this.state.searchWord !== '' ? '&searchWord=' + encodeURIComponent(this.state.searchWord) : '');

        /* 테스트코드
        const url = DEFAULT_CONSTANTS.apiAdminDomain
            + '/v1/app/freeStudy/studyFile/' + 20060001
            + '?page=' + this.state.currentPage
            + '&paginate=' + paginate
            + (this.state.searchWord !== '' ? '&searchWord=' + encodeURIComponent(this.state.searchWord) : '');
        */

        //console.log('getMaterialList()', 'url = ' + url)

        console.log('FreeDataMaterialList.js > getMaterialList()', 'url = ' + url)

        await CommonUtil.callAPI(url)
            .then(response => {

                console.log('FreeDataMaterialList.js > getMaterialList()', 'response = ' + JSON.stringify(response))
                ...
                if (response && response.code === '0000') {
                    ...
                    // TODO 필터링 과목 종합
                    ([...this.state.materialItems, ...response.data.studyFiles.data]).map((item) => {
                    ...
                    this.setState({
                        ...
                        materialItems: [...this.state.materialItems, ...response.data.studyFiles.data],
                        lastPage: response.data.studyFiles.last_page,
                    })
                } else {
                    if(this.state.currentPage == 1) {
                        this.setState({
                    ...
                    response.message
                        ...
                        : Toast.show('학습자료 목록을 불러오는데 실패 했습니다.');
                }}).catch(error => {
                    if(this.state.currentPage == 1) {
                        this.setState({
                            loading: false,
                    ...
                    console.log(error)
                    Toast.show('시스템 에러: 학습자료 목록을 불러오는데 실패 했습니다.');
                });
    }
            ...
            return (
                <SafeAreaView style={styles.container}>
                    ...
                    <View style={styles.content}>
                        {
                            this.state.materialItems.length == 0
                            &&
                            ...
                            ||
                            this.state.materialItems.filter((item) => {
                                ...
                                return (
                                    <TouchableOpacity
                                        ...
                                        onPress={() => this.props.navigation.navigate('FreeDataMaterialDetail', {
                                            freeStudyIdx: item.freeStudyIdx,
                                        })}>
