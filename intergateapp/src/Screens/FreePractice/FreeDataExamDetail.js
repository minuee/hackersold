...
export default class FreeDataExamDetail extends Component {
    constructor(props) {
        super(props);

        this.state = {
            loading: true,
            preExamIdx: this.props.navigation.getParam('preExamIdx'),
            targetItem: {},//this.props.navigation.getParam('examItem'),
        }


        console.log('FreeDataExamDetail > constructor()', 'this.props.navigation = ' + JSON.stringify(this.props.navigation))
    }
    ...
    getExamItem = async() => {
        const url = DEFAULT_CONSTANTS.apiAdminDomain
            + '/v1/app/freeStudy/preExam/view/' + this.state.preExamIdx


        console.log('FreeDataExamDetail.js > getExamItem()', 'url = ' + url)

        await CommonUtil.callAPI(url)
            .then(response => {

                console.log('FreeDataExamDetail.js > getExamItem()', 'response = ' + JSON.stringify(response))

                if (response && response.code === '0000') {
                    this.setState({
                        targetItem: response.data.preExam || {},
                    })
                } else {
                    response.message
    ...
    downloadFile = (fileItemIndex) => {
        if ( this.state.targetItem.files[fileItemIndex].state === STATE_DOWNLOAD_READY ) {
            //원본코드 let fileId = this.generateFileName(fileItemIndex);
            //원본코드 let fileName = fileId + '.' + this.state.targetItem.files[fileItemIndex].appendExt;
            ...
            return (
                <View style={styles.container}>
                    <FreeParallaxScrollView
                        ...
                        centerTitle={!CommonUtil.isEmpty(this.state.targetItem.title) ? this.state.targetItem.title : ''}
