...
class IntroScreen extends React.Component {
    ...
    constructor(props) {       
        ...
        console.log('introscreen', this.props.screenState.lectureInfo)
        /*
        this.props.screenProps.navigation.addListener('didFocus', payload=>{
            this.setState({
                selectBookIndex : this.props.selectBook.selectBookIndex ? this.props.selectBook.selectBookIndex : 1,
                selectTheme : this.props.selectBook.selectTheme ? this.props.selectBook.selectTheme : 'free',
            })
            this.forceUpdate();
        })
        */
        this.state  = {
            selectBookIndex : this.props.screenState.selectBookIndex,
            selectTheme : this.props.screenState.selectTheme,
            remainTime : this.props.remainTime,
            isLoading : true,
            lectureInfo : this.props.screenState.lectureInfo
        }
    }

    
    UNSAFE_componentWillMount() {
        console.log('UNSAFE_componentWillMount :1111 ');
    }   
        ...
        if ( this.state.isLoading) {
            ...
            return (
                <View style={styles.container}>
                    <NavigationEvents
                        onWillFocus={payload => {
                           this.props.screenState.refreshTextBookInfo()
                        }}                        
                    />
                    <ScrollView style={styles.MainContainer}>     
                        <View style={styles.headerTitle}>           
                            <Text style={[CommonStyle.mt20,CommonStyle.font18]}>선택한 교재는 {this.props.screenState.lectureInfo.Title}</Text>
                        </View>
                        <View style={styles.headerTitle}>           
                        ...
                        <View style={[styles.headerTitle,CommonStyle.mt20]}>
                            <View style={[{flex:1,width:'98%'},styles.ButtonWrap]}>
                                <View style={{flex:1,paddingHorizontal:5}}>
                                    <Button
                                        title="START"
                                        buttonStyle={{flex:1,backgroundColor:'#173f82'}}
