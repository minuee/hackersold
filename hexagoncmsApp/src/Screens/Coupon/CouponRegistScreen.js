...
class CouponRegistScreen extends Component {
    ...
    registData = async() => {
        ...
        try {            
            ...
            const token = this.props.userToken.apiToken;
            let sendData = {
                coupon_type : this.state.couponCode,
                issue_reaseon :  this.state.formReaseon,
                price : this.state.couponCode,
        ...
        if ( this.state.loading ) {
            ...
            return(
                <SafeAreaView style={ styles.container }>
                    <KeyboardAvoidingView style={{paddingVertical:10}} behavior={Platform.OS === 'ios' ? "padding" : 'position'}  enabled={false}> 
                    {
